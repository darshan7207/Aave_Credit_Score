...
GRAPH_URL = "https://api.thegraph.com/subgraphs/name/harsh-eth/compound-v2-ethereum"


ACTIONS = {
    "borrow": ("borrows", "borrower"),
    "repay": ("repays", "payer"),
    "deposit": ("mints", "minter"),
    "redeem": ("redeems", "redeemer"),
    "liquidationcall": ("liquidations", "liquidator")
}


def fetch_transactions(wallet):
    result = []
    for action, (gql_entity, filter_field) in ACTIONS.items():
     query = f"""
    {{
      {gql_entity}(where: {{{filter_field}: "{wallet.lower()}"}}, first: 1000) {{
        id
        blockTimestamp
      }}
    }}
    ...
    if response.status_code == 200:
        ...
        if "data" not in json_resp:
            print(f" No data returned for wallet: {wallet} action: {action}")
            print("Response:", json_resp)
            
        entries = json_resp["data"][gql_entity]
        print(f"{len(entries)} {action} records for {wallet}")
        ...
...
def main():
    ...
    with open("user-wallet-transactions.json", "w") as f:
        json.dump(all_txns, f, indent=2)
    print("Data saved to user-wallet-transactions.json")
